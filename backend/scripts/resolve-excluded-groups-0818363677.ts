...
async function fetchAllConversations(pageId: string, token: string): Promise<PancakeConversation[]> {
  ...
  for (let page = 0; page < 1000; page++) {
    ...
    let data: { conversations?: PancakeConversation[] };
    try {
      data = await pancakeGet<{ conversations?: PancakeConversation[] }>(
        `/api/public_api/v2/pages/${encodeURIComponent(pageId)}/conversations?order_by=updated_at${query}`,
        token,
      );
    } catch (err) {
      console.warn(`Pagination error at page ${page} (cursor=${cursor}), stopping early: ${err instanceof Error ? err.message : err}`);
      break;
    }
...
async function main() {
  ...
  if (pageId !== 'pzl_2132817647513376826') {
    throw new Error(`Token page_id mismatch: expected pzl_2132817647513376826, got ${pageId}`);
  }
  ...
  for (const item of normalizedExclusions) {
    ...
    if (matches.length === 1) {
      ...
      unmatchedGroups.push({ index: item.index, targetName: item.originalName });
    }
  }
  ...
  const outputPath = path.join(process.cwd(), 'excluded_groups_checkpoint_0818363677.json');
