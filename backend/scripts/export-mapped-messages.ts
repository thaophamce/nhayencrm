...
async function main() {
  ...
  // Lấy mapping từ external_thread_id sang conversation_id trên VPS
  const vpsConvMapping = await vpsClient.query(`
    SELECT id, external_thread_id
    FROM conversations
    WHERE zalo_account_id = '75531187-1d0f-44d9-a75b-08cb0c8204c3'
  `);

  const threadIdToVpsConvId = new Map();
  for (const row of vpsConvMapping.rows) {
    threadIdToVpsConvId.set(row.external_thread_id, row.id);
  }

  console.log(`Có ${threadIdToVpsConvId.size} conversations trên VPS`);

  // Lấy messages mới từ LOCAL
  ...
  for (const msg of messages.rows) {
    // Map conversation_id từ LOCAL sang VPS
    const vpsConvId = threadIdToVpsConvId.get(msg.external_thread_id);

    if (!vpsConvId) {
      console.log(`Skip message ${msg.id} - không tìm thấy conversation cho thread ${msg.external_thread_id}`);
      skipped++;
      continue;
    }
    ...
    const values = [
      msg.id ? `'${msg.id}'` : 'NULL',
      `'${vpsConvId}'`, // Dùng VPS conversation_id
      ...
      msg.attachments ? `'${escape(JSON.stringify(msg.attachments))}'::jsonb` : 'NULL',
      ...
      msg.quote ? `'${escape(JSON.stringify(msg.quote))}'::jsonb` : 'NULL',
      ...
      msg.metadata ? `'${escape(JSON.stringify(msg.metadata))}'::jsonb` : 'NULL',
      msg.automation_task_id ? `'${msg.automation_task_id}'` : 'NULL',
      msg.automation_step_index !== null ? msg.automation_step_index : 'NULL',
      msg.mentions ? `'${escape(JSON.stringify(msg.mentions))}'::jsonb` : 'NULL',
  ...
  fs.writeFileSync('scripts/mapped-messages-insert.sql', msgLines.join('\n'));
  console.log(`Đã export ${msgLines.length} messages (skipped ${skipped}) vào scripts/mapped-messages-insert.sql`);
