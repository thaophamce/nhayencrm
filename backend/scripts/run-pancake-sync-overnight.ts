...
const ACCOUNT_ID = '2ba842fc-bee1-43b8-8d68-59211788570f';
const EXPECTED_ZALO_UID = '856188217731117740';
const FROM_DATE_STR = '2026-06-01T00:00:00+07:00';
const FROM_DATE = parsePancakeDate(FROM_DATE_STR);

const CHECKPOINT_FILE = path.join(process.cwd(), 'pancake_sync_checkpoint_thiepcuoi_0913980993.json');
const EXCLUDED_FILE = path.join(process.cwd(), 'excluded_groups_checkpoint_0913980993.json');
...
async function main(): Promise<void> {
  ...
  if (account.status !== 'connected') throw new Error(`Target Zalo account is not connected: ${account.status}`);
  if (account.zaloUid !== EXPECTED_ZALO_UID || account.phone !== '0913980993') {
  ...
  // Dedicated Retry Phase for Error Conversations
