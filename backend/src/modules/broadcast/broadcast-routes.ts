...
//   GET    /broadcasts/helpers/preset-segments     list pre-set segments
//   GET    /broadcasts/helpers/customer-lists      lookup CRM lists
//   GET    /broadcasts/helpers/tags                lookup Tag CRM v2
//   GET    /broadcasts/helpers/nicks               lookup nicks trong quyền
...
import { blockVisibilityWhere } from '../block/block-visibility.js';
import { resolveSegmentToContactIds } from './segment-resolver.js';
import { PRESET_SEGMENTS } from './broadcasts-preset-segments.js';
...
export async function broadcastRoutes(app: FastifyInstance): Promise<void> {
  ...
  // getZaloScope (nick trong quyền) + ẩn nick đã xóa mềm.
  ...
  app.post(BASE, async (request: FastifyRequest, reply: FastifyReply) => {
    try {
      ...
      // Chỉ gửi loạt bằng Khối mình ĐƯỢC THẤY (không dùng khối riêng tư người khác).
      ...
      // Validate pacing.selectedNickIds — bắt buộc ≥1, thuộc org
