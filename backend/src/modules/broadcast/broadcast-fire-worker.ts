...
//
// Worker lifecycle (Queue/Redis wiring) sống ở broadcast-queue.ts — module này
// chỉ export processBroadcastTick làm processor, theo pattern self-contained
// Community (group-scan-queue.ts).
...
import { type BroadcastFireJobData, buildBroadcastTickJobId, getBroadcastFireQueue } from './broadcast-queue.js';
...
async function pickAvailableNicks(orgId: string, selectedNickIds: string[]): Promise<Array<{ id: string; sentToday: number }>> {
  ...
  const results: Array<{ id: string; sentToday: number }> = await Promise.all(
...
// ── BullMQ processor entrypoint — wraps tenant context per job ──────────
export function broadcastTickProcessor(job: Job<BroadcastFireJobData, BroadcastFireResult>): Promise<BroadcastFireResult> {
  return withTenant(job.data.orgId, () => processBroadcastTick(job));
}
