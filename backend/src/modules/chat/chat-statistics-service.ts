...
export async function getChatStatistics(input: ChatStatisticsInput) {
  ...
  const [sentRows, receivedRows, unread, friendRequests, responseRows, hourRows] = await Promise.all([
...
function emptyStatistics(accounts: Array<Record<string, unknown>>) {
  return {
    ...
    totals: { sent: 0, received: 0, total: 0, friendRequests: 0, unread: 0, averageResponseSeconds: 0 },
