...
export async function deliveryRoutes(app: FastifyInstance) {
  ...
  app.get('/api/v1/delivery/business-analytics', {preHandler:[authMiddleware,requireGrant('delivery_business','access')]}, async (request, reply) => { try { return await service.analytics(request.user!, request.query as any); } catch (e: any) { return reply.status(400).send({ error: e.message }); } });
  app.get('/api/v1/delivery/activity', {preHandler:[authMiddleware,requireGrant('delivery','view_all')]}, async (request) => service.activity(request.user!, request.query as any));
