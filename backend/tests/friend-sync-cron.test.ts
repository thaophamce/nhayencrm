...
describe('runFriendSyncCycleNow', () => {
  ...
  it('iterates each connected account sequentially', async () => {
    ...
    syncAccountFullyMock.mockResolvedValue({
      friends: { liveCount: 0, createdContacts: 0, upsertedFriends: 0, emittedCount: 0, errors: 0, durationMs: 1, skipped: null },
  ...
  it('continues iteration when 1 account fails', async () => {
    ...
    syncAccountFullyMock
      ...
      .mockResolvedValueOnce({
        friends: { liveCount: 5, createdContacts: 0, upsertedFriends: 5, emittedCount: 1, errors: 0, durationMs: 50, skipped: null },
  ...
  it('accumulates emittedCount + errors across accounts', async () => {
    ...
    syncAccountFullyMock
      .mockResolvedValueOnce({
        friends: { liveCount: 10, createdContacts: 0, upsertedFriends: 10, emittedCount: 3, errors: 1, durationMs: 100, skipped: null },
        aliasesUpdated: 1, labelsUpdated: 2, errors: [], durationMs: 100,
      })
      .mockResolvedValueOnce({
        friends: { liveCount: 5, createdContacts: 1, upsertedFriends: 5, emittedCount: 2, errors: 0, durationMs: 80, skipped: null },
        aliasesUpdated: 0, labelsUpdated: 1, errors: [], durationMs: 80,
      });
