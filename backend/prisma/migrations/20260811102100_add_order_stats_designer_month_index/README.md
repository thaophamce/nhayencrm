# Order statistics designer/month index runbook
...
WHERE ns.nspname = current_schema()
  AND cls.relname = 'orders_org_id_designer_id_created_at_idx';
```
...
```sql
DROP INDEX CONCURRENTLY IF EXISTS "orders_org_id_designer_id_created_at_idx";
```

Then clear Prisma's failed-migration marker before retrying:

```bash
npx prisma migrate resolve --rolled-back 20260811102100_add_order_stats_designer_month_index
```
...
```bash
npx prisma migrate resolve --applied 20260811102100_add_order_stats_designer_month_index
```

After deploy, repeat the check and require both flags to be true. Expected columns
are `org_id, designer_id, created_at`. Emergency rollback is the same concurrent
`DROP INDEX`; record rollback as a new forward migration after a successful deploy.
