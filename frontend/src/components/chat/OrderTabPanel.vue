  ...
  <div class="order-panel">
    ...
    <main class="order-scroll">
      ...
      <section class="order-section payment-section">
        ...
        <label><input v-model="bankTransfer" type="checkbox" /> Chuyển khoản</label>
    ...
    <footer class="order-footer"><strong>{{ money(total) }}</strong><div><button @click="reset">Thiết lập lại</button><button :disabled="submitting || !!linkedOrder || !!syncedOrder" @click="createPancakeOrder">{{ submitting ? 'Đang tạo…' : linkedOrder ? 'Đã tạo đơn' : 'Tạo đơn' }}</button></div></footer>
  </div>
</template>
...
const freeShipping=ref(false), bankTransfer=ref(false), shippingFee=ref(0), discount=ref(0), note=ref(''), printNote=ref('');
...
function applyPancakeOrder(order:any){
  ...
  freeShipping.value=!!order.freeShipping; bankTransfer.value=Number(order.transferMoney)>0;
...
async function createPancakeOrder(){if(submitting.value)return;if(!customer.value.name.trim())return toast.error('Nhập tên khách hàng');submitting.value=true;try{const payload={warehouse_id:warehouseId.value,bill_full_name:customer.value.name.trim(),bill_phone_number:customer.value.phone.trim(),shipping_address:customer.value.address.trim()?{address:customer.value.address.trim(),full_address:customer.value.address.trim(),full_name:customer.value.name.trim(),phone_number:customer.value.phone.trim()}:undefined,note_print:printNote.value||note.value,items:items.value.map(i=>({variation_id:i.variation_id,product_id:i.product_id,quantity:i.quantity,variation_info:{name:i.name,detail:i.detail,retail_price:i.price}})),shipping_fee:freeShipping.value?0:shippingFee.value,total_discount:discount.value,is_free_shipping:freeShipping.value,transfer_money:bankTransfer.value?total.value:0,cash:bankTransfer.value?0:total.value};const {data}=await api.post(`/orders/pancake/from-conversation/${props.conversationId}`,payload);linkedOrder.value=data.link;toast.success(data.renameSucceeded&&props.threadType==='group'?`Đã tạo đơn ${data.link?.orderCode} và đổi tên nhóm`:`Đã tạo đơn ${data.link?.orderCode}`)}catch(e:any){toast.error(e.response?.data?.error||'Tạo đơn thất bại');await loadLink()}finally{submitting.value=false}}
...
function reset(){items.value=[];shippingFee.value=0;discount.value=0;freeShipping.value=false;bankTransfer.value=false;note.value='';printNote.value='';prefill()}
