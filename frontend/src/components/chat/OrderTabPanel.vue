  ...
  <div class="order-panel">
    ...
    <main class="order-scroll">
      ...
      <section class="order-section products-section">
        ...
        <div class="product-table">
          ...
          <div v-for="(item,index) in items" :key="item.variation_id" class="product-row">
            ...
            <input :value="item.quantity" type="text" inputmode="numeric" class="quantity-input" @input="setQuantity(item, $event)" />
            <input
        ...
        <div class="product-search">
          <v-icon size="17">mdi-magnify</v-icon><input v-model="search" placeholder="Tìm kiếm sản phẩm" @keydown="onProductKeydown" />
        ...
        <div v-if="showProducts" class="product-results">
          ...
          <button v-for="(p, productIndex) in products" :key="p.id" :class="{ active: productIndex === highlightedProductIndex }" @mouseenter="highlightedProductIndex = productIndex" @click="addProduct(p)"><span><b>{{ p.product?.name || p.display_id }}</b><small>{{ variationDetail(p) }}</small></span><strong>{{ money(p.retail_price) }}</strong></button>
      ...
      <section class="order-section payment-section">
        ...
        <div class="sum-row"><span>Tổng giá trị đơn hàng</span><b class="money-red">{{ money(subtotal) }}</b></div>
        <div class="sum-row"><span>Đã đặt cọc</span><input v-model.number="deposit" type="number" min="0" :max="total" /></div>
        <div class="sum-row amount-due"><span>Tiền cần thu</span><b class="money-red">{{ money(amountDue) }}</b></div>
      </section>
...
function applyPancakeOrder(order:any){
  ...
  deposit.value=Number(order.qrPay)||0;
...
async function loadProducts(){const term=search.value.trim();const requestId=++productRequestId;if(!term){products.value=[];showProducts.value=false;highlightedProductIndex.value=-1;return}productLoading.value=true;showProducts.value=true;try{const {data}=await api.get('/orders/pancake/products',{params:{search:term,limit:30}});if(requestId===productRequestId){products.value=data.products||[];highlightedProductIndex.value=products.value.length?0:-1}}catch(e:any){if(requestId===productRequestId)toast.error(e.response?.data?.error||'Không tải được sản phẩm')}finally{if(requestId===productRequestId)productLoading.value=false}}
...
.product-columns,.product-row{grid-template-columns:2.2fr .72fr 1fr 1fr}
